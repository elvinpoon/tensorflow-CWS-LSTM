...
from utils import CharEmbedding, data_loader, batch_iter
import sys
import os
import re
import pickle
...
class CWSModel(object):
    def __init__(self, is_training, config):
        ...
        self._input_data = tf.placeholder(tf.int32, [None, num_steps,num_input])
        self._target = tf.placeholder(tf.float32, [None, num_steps,num_class])
    ...
    def prediction(self):
        ...
        if self.config.model_type == "LSTM":
            ...
            outputs, state = tf.nn.rnn(cell, unpack_sequence(inputs),
        ...
        for ids, x in enumerate(outputs):
            ...
            deviation = tf.tile(tf.expand_dims(tf.reduce_min(previous, reduction_indices=1), 1),
                                [1, num_class])
            previous -= deviation
            focus = 1.
            ...
            previous = prediction
            mask = tf.less(tf.mul(tf.ones([batch_size], dtype=tf.int32), ids + 1), self._length)
...
def run_epoch(session, model, data, eval_op,char_embedding, verbose=True):
    ...
    error_rate = 0
    delta_t = time.time()
    for step, (x, y, length) in enumerate(batch_iter(data, model.config.batch_size, model.config.num_steps, char_embedding,
        ...
        if verbose and not model.is_training:
            ...
            for xds in range(model.config.batch_size):
                ...
                for j in range(sss):
                    ...
                    if r == 0:
                        continue
                    print index2word[x[xds][j][1]], trans[r], ttts[int(pred[xds][j])] == trans[r]
                print '<EOS>'
            sys.stderr.write()
